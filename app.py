    ...
    try:
        ...
        for resource_name, resource_details in yaml_data.get("Resources", {}).items():
            if resource_details.get("Type") == "AWS::EC2::SecurityGroup":
                for ingress_rule in resource_details.get("Properties", {}).get(
       ...
       for rule_name, rule_condition, rule_details in criteria:
           try:
               if rule_condition(resource_details):
                   issues.append(
                       (f"{rule_name} issue in resource '{resource_name}'", rule_details)
                   )
           except Exception as e:
               logging.error(
                   f"Error evaluating rule '{rule_name}' for resource '{resource_name}': {str(e)}"
               )
...
def show_result(template_path, criteria):
    try:
        if template_path:
            ...
            result_output = print_issues(identified_issues, pipeline_fail_threshold=1)
            ...
            s3.upload_file(
                "pipeline_report.txt", "project-log-details", "pipeline_report.txt"
            )
...
def open_code():
    try:
        ...
        save_button = tk.Button(
            code_frame, text="Save", command=lambda: save_changes(code_text)
        )
        save_button.pack(side=tk.LEFT, padx=5, pady=5)

        discard_button = tk.Button(
            code_frame, text="Discard", command=lambda: discard_changes(code_text)
        )
...
def show_template_selection_gui(root):
   try:
       ...
       tk.Checkbutton(
           root, text="Security Group Traffic check", variable=sg_traffic_var
       ).pack()
       ...
       tk.Button(
           ...
           command=lambda: run_analysis(
               cidr_ip_var, ssh_traffic_var, all_traffic_var, sg_traffic_var
           ),
       ).pack()
...
def run_analysis(
    cidr_ip_var, ssh_traffic_var, all_traffic_var, sg_traffic_var, template_path
):
   try:
       criteria = []
       ...
       if (
               not cidr_ip_var.get()
               and not ssh_traffic_var.get()
               and not all_traffic_var.get()
               and not sg_traffic_var.get()
       ):
           criteria.append(
               (
                   "Security",
                   lambda resource: "SecurityGroup" in resource.get("Type", "")
                                    and any(
                       rule.get("CidrIp", "") == "0.0.0.0/0"
                       for rule in resource.get("Properties", {}).get(
                           "SecurityGroupIngress", []
                       )
                   ),
                   "Security group allows traffic from '0.0.0.0/0'",
           ...
           criteria.append(
               (
                   ...
                   lambda resource: "SecurityGroup" in resource.get("Type", "")
                                    and any(
                       (rule.get("FromPort", 0) == 22 or rule.get("ToPort", 0) == 22)
...
home_label = ttk.Label(
    root,
    text="Welcome To Security Testing Tool",
...
description_label.pack(pady=10, anchor="s")
description_label = ttk.Label(
    home_page, text="The Tool contain following featuress.", font=("Helvetica", 11)
)
description_label.pack(pady=10, anchor="s")
...
for point in bullet_points:
    bullet_label = ttk.Label(
        home_page, text="• " + point, font=("Helvetica", 11), style="Bullet.TLabel"
    )
    bullet_label.pack(anchor="w")

# Button to edit code
edit_code_button = tk.Button(
    customization_page, text="Edit Code", command=open_code, font=("Helvetica", 14)
)
edit_code_button.pack(pady=10)

# Run Code page elements
run_code_label = ttk.Label(
    run_code_page, text="Run Code", font=("Helvetica", 20, "bold")
)
run_code_label.pack(pady=24)

# Show Report page elements
show_report_label = ttk.Label(
    show_report_page, text="Show Report", font=("Helvetica", 20, "bold")
)
show_report_label.pack(pady=24)

# Show Log page elements
show_log_label = ttk.Label(
    show_log_page, text="Show Log", font=("Helvetica", 20, "bold")
)
show_log_label.pack(pady=24)

# Button to open logs in the GUI frame
open_logs_button = tk.Button(
    show_log_page, text="Open Logs", command=open_logs, font=("Helvetica", 12)
)
open_logs_button.pack(pady=10, anchor="center")

# Button to open reports in the GUI frame
open_reports_button = tk.Button(
    show_report_page, text="Open Reports", command=open_reports, font=("Helvetica", 12)
)
...
tk.Checkbutton(
    ...
    variable=cidr_ip_var,
).pack(anchor="center")
tk.Checkbutton(
    run_code_page,
    text="SSH Traffic check",
    font=("Helvetica", 10, "bold"),
    variable=ssh_traffic_var,
).pack(anchor="center")
tk.Checkbutton(
    run_code_page,
    text="All Traffic check",
    font=("Helvetica", 10, "bold"),
    variable=all_traffic_var,
).pack(anchor="center")
tk.Checkbutton(
    run_code_page,
    text="Security Group Traffic check",
    font=("Helvetica", 10, "bold"),
    variable=sg_traffic_var,
).pack(anchor="center")

# Button to select template
select_template_button = tk.Button(
    run_code_page,
    text="Select Template",
    font=("Helvetica", 12),
    command=lambda: select_template(
        root,
        cidr_ip_var,
        ssh_traffic_var,
        all_traffic_var,
